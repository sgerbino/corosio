...
struct kqueue_faults
{
    void testConstructorFails()
    {
        auto expect_throw = [](sys s, unsigned nth, int err, std::errc code)
        {
            ...
            // before throwing (kqueue_scheduler.hpp:185-199).
    ...
    void testOpenFails()
    {
        ...
        // order (kqueue_traits.hpp:167-172).
        ...
        }
        {
            ...
            // spells its writes writev() (kqueue_traits.hpp:80) and
            // write() (kqueue_traits.hpp:95), neither of which carries
        ...
        // function of its own (kqueue_traits.hpp:209-214), which has
        // its own error leg to leave through.
        {
            int before = open_fds();
    ...
    void testAssignRegisterFails()
    {
        ...
        make_native_adoptable(h);
        {
            ...
            // (reactor_basic_socket.hpp:106-113).
            tcp_socket s(ioc);
            int held = open_fds();
            fault_scope f(sys::kevent, ENOMEM);
            auto ec = s.assign(h);
    ...
    void testAcceptFails()
    {
        ...
        auto body = [&]() -> capy::task<>
        {
            ...
            }
            {
                ...
                // accept succeeds (kqueue_traits.hpp:108-114).
    ...
    void testAcceptConfigureFails()
    {
        ...
        auto body = [&]() -> capy::task<>
        {
            ...
            for(unsigned nth = 1; nth <= 3; ++nth)
            {
                ...
                // accepted fd (kqueue_traits.hpp:119-135).
                fault_scope f(sys::fcntl, EINVAL, nth);
            ...
            }
            {
                ...
                // same reason (kqueue_traits.hpp:141-150).
                fault_scope f(sys::setsockopt, ENOPROTOOPT);
                auto [ec] = co_await acc.accept(s);
                sockopt_ec = ec;
    ...
    void testRunLoopFaults()
    {
        ...
        // (reactor_scheduler.hpp:829-834), so nth 1 is that wait.
        {
            io_context ioc(kqueue);
    ...
    */
    void testInterruptTriggerFails()
    {
        io_context ioc(kqueue);
        {
            ...
            // a reactor thread (reactor_scheduler.hpp:572-581), and it
    ...
    void testSignalReaderRegisterFails()
    {
        in_child([]{
            ...
            bool fired = false;
            {
                ...
                // (posix_signal_service.hpp:565-571).
                fault_scope f(sys::kevent, ENOMEM);
                ec = ss.add(SIGUSR2);
                fired = f.fired();
            }
