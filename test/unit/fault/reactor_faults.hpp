...
struct reactor_common_faults
{
    ...
    // symbol to arm differs. kqueue spells the single-buffer fast path
    // write() (kqueue_traits.hpp:89-99) and the vector form writev()
    // (kqueue_traits.hpp:75-84); epoll and select spell them
    // send(MSG_NOSIGNAL) and sendmsg() (epoll_traits.hpp:73,85,
    // select_traits.hpp:84,101), falling back to write() where the
    // platform has no MSG_NOSIGNAL. Darwin defines it, so select on
    // macOS arms exactly as it does on Linux.
    ...
    /* Connect a pair whose sender cannot outrun its peer.
       ...
       longer binds (test/unit/wait.cpp:169-177). The accepted socket
    ...
    */
    ...
    void testConnectFails()
    {
        ...
        // probe and the SO_ERROR read at reactor_op.hpp:154,175 are
        // reached -- not that a reactor dispatch delivered them.
    ...
    void testDeferredWriteFails()
    {
        ...
        // op's reactor retry (reactor_op.hpp:356).
