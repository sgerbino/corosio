...
`std::runtime_error` naming the failing step; the underlying
`error_code::message()` appears in the exception text for bind and
listen failures, and on `stderr` for accept and connect failures.
