...
Acceptors are the exception: `wait(wait_type::write)` on an acceptor
completes immediately on the reactor and IOCP backends and never
completes on `io_uring`, because writability has no meaning for a
listening socket. Do not use it.
